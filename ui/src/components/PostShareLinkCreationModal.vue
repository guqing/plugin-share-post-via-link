...
import { PostShareLinkSpecShareTypeEnum, type PostShareLink } from '@/api/generated'
...
async function onSubmit(data: PostShareLinkFormState) {
  ...
  try {
    ...
    const { data: newPostShareLink } = await postShareApiClient.postShare.createPostShareLink({
      postShareLink: {
        kind: 'PostShareLink',
...
const showAlert = useLocalStorage('plugin:share-post-via-link:show-creation-tips', true)
...
<template>
  ...
  >
    <div>
      <div v-if="showAlert" class=":uno: mb-5">
        <VAlert
          type="info"
          title="提示"
      ...
      <div v-if="existLinks?.total" class=":uno: mb-5">
        <VAlert type="default" title="当前文章已存在分享链接" :closable="false">
          <template #description>
            <ol class=":uno: list-decimal list-inside">
              <li v-for="existLink in existLinks?.items" :key="existLink.metadata.name">
          ...
          <template #actions>
            <VButton type="secondary" size="sm" @click="$router.push({ name: 'SharedPostLinks' })">
              去管理
            </VButton>
          </template>
        </VAlert>
